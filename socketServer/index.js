...
cam_nsp.use(middleware);									//Khi webapp emit bất kỳ lệnh gì lên thì sẽ bị bắt

server.listen(PORT);										// Cho socket server (chương trình mạng) lắng nghe ở port 3484
console.log("Server nodejs IP: " + ip.address() + ":" + PORT)
...
webapp_nsp.on('connection', function(socket) {	
	...
	socket.on('*', function(packet) {
		...
		cam_nsp.emit(eventName, eventJson)
	});
})
// bắt sự kiện của CAM client
cam_nsp.on("*",function(packet) {
	...
	socket.on('*', function(packet) {
		console.log("Cam rev and send to webapp packet: ", packet.data) //in ra để debug
		var eventName = packet.data[0]
		var eventJson = packet.data[1] || {} //nếu gửi thêm json thì lấy json từ lệnh gửi, không thì gửi chuỗi json rỗng, {}
		webapp_nsp.emit(eventName, eventJson) //gửi toàn bộ lệnh + json đến esp8266
	});
})